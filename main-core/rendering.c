...
*/
{
	...
	x_ratio = (double)src_img->width / new_width;
	y_ratio = (double)src_img->height / new_height;
	y = 0;
	while (y < new_height)
	{
		x = 0;
		while (x < new_width)
		{
			*(int *)(new_img->data + (y * new_img->size_line + x
					...
					+ ((y * y_ratio) * src_img->size_line
						+ (x * x_ratio) * (src_img->bpp / 8)));
			x++;
		}
		y++;
	}
...
*/
{
	if (data->map[y][x].type == '1')
		...
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->collectibles->image, x * data->cell_size,
			y * data->cell_size);
	else if (data->map[y][x].type == 'P' && (data->player_x % 2 == 0))
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->player1->image, x * data->cell_size, y * data->cell_size);
	else if (data->map[y][x].type == 'P' && (data->player_x % 2 != 0))
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
